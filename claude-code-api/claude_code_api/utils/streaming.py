...
class OpenAIStreamConverter:
    ...
    def _split_into_segments(text: str) -> List[str]:
        """Split text into sentence-level segments for progressive streaming."""
        # Split on sentence boundaries: period/exclamation/question followed by space or newline
        parts = re.split(r'(?<=[.!?。！？])\s+|(?<=\n)', text)
        segments: List[str] = []
        for part in parts:
            if part:
                segments.append(part if part.endswith(('\n', ' ')) else part + ' ')
        # If only one segment or very short text, return as-is
        if len(segments) <= 1 or len(text) < 40:
            return [text]
        return segments
    ...
    def _assistant_chunks(self, message: Any) -> Tuple[List[str], bool, bool]:
        ...
        if text_content:
            # Split into sentence-level segments for progressive streaming
            segments = self._split_into_segments(text_content)
            for segment in segments:
                chunks.append(
                    SSEFormatter.format_event(self._build_chunk({"content": segment}))
                )
            saw_text = True
            ...
            async for claude_message in claude_process.get_output():
                ...
                if self.parser.is_assistant_message(message):
                    ...
                    for i, chunk in enumerate(chunks):
                        yield chunk
                        # Small delay between segments for progressive display
                        if saw_text and i < len(chunks) - 1:
