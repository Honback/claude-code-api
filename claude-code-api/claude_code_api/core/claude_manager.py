...
class ClaudeProcess:
    ...
    async def send_input(self, text: str):
        ...
        Note: stdin is closed after process start (prompt is passed via -p flag),
        so this method is currently a no-op.  Conversation continuation should
        use --resume with a new process instead.
        """
        logger.warning(
            "send_input called but stdin is closed; use --resume for follow-up",
            session_id=self.session_id,
        )
