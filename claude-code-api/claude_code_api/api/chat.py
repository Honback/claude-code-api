...
async def _check_auth_status() -> bool:
    """Quick auth check before starting a chat session."""
    try:
        from claude_code_api.api.auth import auth_status as _auth_status_fn

        result = await _auth_status_fn()
...
async def create_chat_completion(request: ChatCompletionRequest, req: Request) -> Any:
    ...
    # Pre-check authentication - fail fast if not logged in
    is_authenticated = await _check_auth_status()
    if not is_authenticated:
        raise _http_error(
            status.HTTP_401_UNAUTHORIZED,
            "인증이 필요합니다. Settings 페이지에서 OAuth 로그인 또는 API 키를 설정하세요. "
            "(Authentication required. Please set up OAuth login or API key in Settings.)",
